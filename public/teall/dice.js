...
(function (dice) {
  ...
  this.create_d10_geometry = function (radius) {
    let a = (Math.PI * 2) / 10,
      ...
      v = -1;
    let vertices = [];
    for (let i = 0, b = 0; i < 10; ++i, b += a) vertices.push([Math.cos(b), Math.sin(b), h * (i % 2 ? 1 : -1)]);
    vertices.push([0, 0, -1]);
    vertices.push([0, 0, 1]);
    let faces = [
      [5, 7, 11, 0],
  ...
  this.dice_box.prototype.roll = function (vectors, values, callback) {
    ...
    if (values != undefined && values.length) {
      ...
      this.prepare_dices_for_roll(vectors);
      for (let i in res) shift_dice_faces(this.dices[i], values[i], res[i]);
    }
  ...
  this.dice_box.prototype.search_dice_by_mouse = function (ev) {
    let m = $t.get_mouse_coords(ev);
    let intersects = new THREE.Raycaster(
  ...
  this.dice_box.prototype.draw_selector = function () {
    ...
    for (let i = 0, pos = -3; i < that.known_types.length; ++i, ++pos) {
      let dice = $t.dice['create_' + that.known_types[i]]();
  ...
  function throw_dices(box, vector, boost, dist, notation_getter, before_roll, after_roll) {
    let uat = $t.dice.use_adapvite_timestep;
